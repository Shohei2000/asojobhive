<div class="m-tab-navigation row w-100 align-items-center">
    ...
    <div class="col-sm-2 text-center m-tab-navigation-col {{$active === 'JobDetail' ? 'm-tab-navigation-active' : ''}}">
    ...
    <div class="col-sm-3 text-center m-tab-navigation-col">
    ...
    <div class="col-sm-3 text-center m-tab-navigation-col">
        <form class="bookmark-right" action="{{ route('bookmark.store') }}" method="POST">
            @csrf
            <button class="btn btn-lg btn-success btn-block w-100 text-white">応募する</button>
            <input type="hidden" name="company_id" value="{{ $company->id }}">
        </form>
    </div>
